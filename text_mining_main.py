...
class TextMiner:
    ...
    def vectorize_tfidf_manual(self, n_docs=5):
        ...
        # 2. TF (Fréquence Relative) => (Nombre d'occurrences du mot) / (Nombre max d'occurrences dans le corpus)

        max_counts_per_doc = raw_counts.max(axis=1)
        max_counts_per_doc[max_counts_per_doc == 0] = 1 # Eviter la division par zero car erreurs
        self.tf_matrix = raw_counts / max_counts_per_doc[:, np.newaxis]

        # 3. IDF (Specificité) => log(Nombre total descriptions / Nombre de descriptions contenant le mot)
        ...
        # 4. TF-IDF Final => TF * IDF
        self.tfidf_matrix = self.tf_matrix * self.idf_vector
        print(f"   > Final TF-IDF matrix ready.")

        # Affichage des têtes des matrices TF, IDF et TF-IDF
        subset_tf = self.tf_matrix[:n_docs, :]
        
        # Sélection des indices des mots actifs dans les premiers documents
        active_indices = np.where(subset_tf.sum(axis=0) > 0)[0]
        
        # Limite à 8 mots pour l'affichage
...
if __name__ == "__main__":
    ...
    if miner.load_and_process():
        ...
        output_name = f".patagonia/patagonia_clustered_{ngram_mode}.xlsx" # à remplacer
