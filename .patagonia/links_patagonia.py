...
async def harvest_links():
    async with async_playwright() as p:
        ...
        await page.goto(TARGET_URL, timeout=60000, wait_until="domcontentloaded") #Le robot ouvre Patagonia et attend que la page soit complètement chargée.
        ...
        try:
            ...
            print("   > No cookie banner") #S'il n'y a pas de bannière, il continue simplement.
        ...
        while True:
            ...
            await asyncio.sleep(2) #Attendre 2 secondes pour que le contenu se charge => Pourquoi ? car les sites modernes ne chargent pas tout d'un coup
            ...
            if current_count > previous_count:
                ...
                retries += 1
                print(f"   > Waiting... ({retries}/3)") #Si aucun nouveau produit n'est chargé après plusieurs tentatives, on arrête
        ...
        unique_links = sorted(list(set([link.split('?')[0] for link in raw_links]))) # Nettoyage simple des doublons d'URL (couleurs)
        
        print(f"   > Found {len(unique_links)} unique products.") #Afficher le nombre de produits uniques trouvés
...
if __name__ == "__main__":
    asyncio.run(harvest_links()) #Lancer la fonction principale pour démarrer le scraping des liens.